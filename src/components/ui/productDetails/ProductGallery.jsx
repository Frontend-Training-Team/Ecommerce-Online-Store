        ...
        overflow-hidden border border-[#EDE8E3] dark:border-line flex items-center justify-center group">
          {activeImage ? (
            <img
              ...
              className="w-full h-full object-cover object-center transition-transform duration-500 group-hover:scale-105 dark:brightness-[.92]"
              onError={(e) => {
                e.target.style.display = "none";
              }}
            />
          ) : (
            <div className="flex flex-col items-center justify-center text-[#A69B91] dark:text-fg-tertiary gap-2">
      ...
      {normalizedImages.length > 1 && (
        <div className="order-2 lg:order-0 lg:col-start-1 lg:row-start-2 relative flex items-center justify-center gap-3 pt-1">
          <button
            ...
            className="w-8 h-8 rounded-full bg-white dark:bg-noir-800 border border-[#D6CFC7] dark:border-line-strong text-[#4A423C] dark:text-fg-secondary hover:bg-[#FAF8F5] dark:hover:bg-noir-750 
            hover:text-[#7E4A2D] dark:hover:text-copper-300 flex items-center justify-center shadow-sm transition-colors cursor-pointer shrink-0"
          >
            ...
            {normalizedImages.map((imgUrl, index) => {
              ...
              return (
                <button
                  ...
                  border-2 transition-all cursor-pointer shrink-0 bg-[#F7F5F2] dark:bg-noir-800 ${isSelected
                      ? "border-[#7E4A2D] dark:border-copper-600 shadow-md scale-105 ring-2 ring-[#7E4A2D]/20 dark:ring-copper-400"
                      : "border-transparent hover:border-[#D6CFC7] dark:hover:border-line-hover opacity-75 hover:opacity-100"
                    }`}
                >
                  <img
                    src={imgUrl}
                    alt={`Thumbnail ${index + 1}`}
                    className="w-full h-full object-cover dark:brightness-[.92]"
                  />
                </button>
              );
            })}
